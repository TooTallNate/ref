...
/**
 * Read a C String (i.e. null terminated) from a Buffer at the given offset.
 */

exports.readCString = function readCString (buffer, offset, encoding) {
  assert(Buffer.isBuffer(buffer), 'Expected Buffer instance')
  if (typeof offset !== 'number') {
    offset = 0
  }
  var zero = -1
  for (var i = offset, l = buffer.length; i < l; i++) {
    if (buffer[i] === 0) {
      zero = i
      break
    }
  }
  assert(zero !== -1, 'Could not find NULL terminator byte in buffer')
  return buffer.toString(encoding || 'utf8', offset, zero)
}
 ...
 * to a some utf8 string data, so we have to create a 2nd buffer...
 */
...
Buffer.prototype.readCString = function readCString (offset, encoding) {
  return exports.readCString(this, offset, encoding)
}
